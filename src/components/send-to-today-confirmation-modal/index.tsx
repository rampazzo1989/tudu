...
import { CalendarStaticIcon, CopyStaticIcon } from '../../assets/static/tudu-icons';
...
const SendToTodayConfirmationModal: React.FC<SendToTodayConfirmationModalProps> = ({ isVisible: isOpen, onReschedule, onCreateCopy, onModalClose }) => {
  ...
  return (
    <PopupModal
      ...
      <OptionsContainer>
        <IconedOptionTile Icon={CalendarStaticIcon} label={t('options.rescheduleForToday')} onPress={onReschedule} iconAnimationDelay={800} />
        <IconedOptionTile Icon={CopyStaticIcon} label={t('options.copyToToday')} onPress={onCreateCopy} iconAnimationDelay={1600} />
      </OptionsContainer>
    </PopupModal>
  );
};
